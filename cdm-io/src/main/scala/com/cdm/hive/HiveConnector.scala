...
class HiveConnector extends Logg with AutoCloseable {
  ...
  private def connectURL(config: HiveConf, hive2WithPort: String, discoveryEnabled: Boolean = false): String = {
    ...
    else s"jdbc:hive2://${config.get("hive2.hosts", "xrdcldbdm010001.unix..net:10000")}/default;principal=${config.get("hive.server2.authentication.kerberos.principal", "hive/_HOST@HCA.CORPAD.NET")}"
  }
